...
2. The per-file loop calls `g.resolver.Resolve(ctx, []string{path})` once per conflicted path. For each path:
   ...
   - On non-nil error: the file is **quarantined** — moved within the working tree from `<path>` to `_conflicts/<path>.<unix-timestamp-seconds>.md`, where the directory tree under `_conflicts/` mirrors the original path, and the timestamp is inserted before the final `.md` extension. If the original path does not end in `.md`, the timestamp is appended after the full original filename with a `.quarantined` suffix (so `foo.bin` becomes `_conflicts/foo.bin.<ts>.quarantined`). The move uses `git rm` on the source + write + `git add` on the destination (git refuses to `git mv` conflicted files), keeping both ends staged in the merge commit.
...
4. If **every** conflicted path failed quarantine itself (e.g. an I/O step in the read+rm+write+add sequence errored on all of them), the existing abort path runs: `git merge --abort` + return `ErrConflictResolutionFailed`. The pod can still wedge in this pathological case — but a single bad file no longer triggers it.
...
| Any I/O step of the quarantine flow fails (read source, git rm source, mkdir dest, write dest, git add dest) | That single path falls back to the existing per-file outcome: counted as a failed quarantine attempt. If at least one other path succeeded (resolved or quarantined) the merge still commits. If every path failed both resolve AND quarantine: run `git merge --abort` + return `ErrConflictResolutionFailed` (existing wedge behavior preserved as the floor) | Operator inspects disk; clears collision or frees space | `git_rest_resolver_failures_total{category="quarantine_io_failed"}` increments (label covers the full read+rm+write+add sequence — git mv is NOT used because git refuses to move conflicted files); ERROR log line names path and the failing step's stderr | Reversible | Same |
...
| Pod crash between staging the quarantine (git rm + git add) and `git commit` | On restart, the existing puller startup path (`recoverAbandonedMerge`) detects `MERGE_HEAD` and runs `git merge --abort`, which undoes the staged quarantine moves. The next pull retries from a clean tree. | Existing puller restart path | Existing pod-restart metrics | Reversible — git's merge state is on-disk and resumable | Single-pod-per-repo deployment ensures no concurrent writer |
| Timestamp collision: two quarantines of the same path in the same second | The second quarantine's destination write fails with "file exists"; falls back to the "quarantine I/O fails" row above | Operator removes one of the duplicates | ERROR log includes both paths | Reversible | Single pull loop guarantees serial execution; collision only possible if operator manually quarantines too |
| Quarantine destination path exceeds filesystem max path length | The destination write fails; falls back to the "quarantine I/O fails" row above | Operator shortens or moves the file | Same | Reversible | Same |
